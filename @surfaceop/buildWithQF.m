function buildWithQF(S, Q)
%BUILDWITHQF   Build the merge tree of patches with quadforest info.
%   TODO edit docs
%   BUILDWITH(S) will construct the global solution operator from the local
...
% Loop over each quadtree:
for i = 1:Q.n_trees
    % Loop over each level of qt's merge_idx
    for j = 1:size(Q.merge_idx{i})
        idxj = Q.merge_idx{i}{j};
        q = cell(size(idxj, 1), 1);
